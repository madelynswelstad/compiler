...
pub fn build_ast(tree: &ParseTree, tokens: &[(String, Option<char>)], idx: &mut usize, alphabet: &[char]) -> Result<AstNode, String> {
    match tree {
        ParseTree::Interior { symbol, children } => {
            match symbol.as_str() {
                "RE"      => build_ast(&children[0], tokens, idx, alphabet),
                "ALT"     => build_alt(children, tokens, idx, alphabet),
                "SEQ"     => build_seq(children, tokens, idx, alphabet),
                "ATOM"    => build_atom(children, tokens, idx, alphabet),
                "NUCLEUS" => build_nucleus(children, tokens, idx, alphabet),
